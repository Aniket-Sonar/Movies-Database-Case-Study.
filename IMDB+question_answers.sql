...
SELECT COUNT(title) AS Movies_produced_in_USA_OR_India_2019
FROM movie
...
WITH count_of_movies_with_one_genre AS (
SELECT movie_id
FROM genre
GROUP BY movie_id HAVING Count(DISTINCT genre) = 1 
)

SELECT COUNT(*) AS count_of_movies_with_one_genre
FROM count_of_movies_with_one_genre;
...
SELECT SUM(total_votes) AS German_movies_votes
FROM movie m 
INNER JOIN 
ratings r
ON m.id = r.movie_id
...
SELECT COUNT(g.movie_id) AS movie_count, genre
FROM genre g
INNER JOIN
ratings r
...
SELECT name AS director_name, COUNT(m.movie_id) AS movie_count
FROM names n
INNER JOIN 
director_mapping m
ON n.id = m.name_id
...
WHERE genre IN ("Comedy", "Drama", "Action") AND avg_rating > 8
GROUP BY name
ORDER BY movie_count DESC;
...
SELECT name AS actor_name, COUNT(m.movie_id) AS movie_count 
FROM names n
INNER JOIN
role_mapping m
ON n.id = m.name_id
...
SELECT name AS actor_name, AVG(total_votes) AS total_votes, COUNT(r.movie_id) AS movie_count,
...
SELECT m.title AS movie, ROUND(AVG(avg_rating), 1) AS avg_rating,
(CASE 
	WHEN avg_rating > 8 THEN "Superhit movies"
    WHEN avg_rating BETWEEN 7 AND 8 THEN  "Hit movies"
    WHEN avg_rating BETWEEN 5 AND 7 THEN  "One-time-watch movies"
	ELSE "Flop movies"
END ) AS "Category_of_thriller_movies"
FROM genre g
INNER JOIN
ratings r
ON r.movie_id = g.movie_id
...
SELECT genre, AVG(duration) AS avg_duration,
SUM(AVG(duration)) OVER (ORDER BY genre ROWS UNBOUNDED PRECEDING)  AS running_total_duartion,
...
WITH movie_data AS (
SELECT genre, year, title AS movie_name, IFNULL(m.worlwide_gross_income, 0) AS worldwide_gross_income,
DENSE_RANK() OVER (PARTITION BY year ORDER BY IFNULL(m.worlwide_gross_income, 0)) AS movie_rank  
FROM movie m
INNER JOIN
genre g
ON m.id = g.movie_id
WHERE (genre LIKE "%Drama%" OR genre LIKE "%Comedy%" OR genre LIKE "%Thriller%" )
)
...
WHERE category = "actress" AND avg_rating > 8 AND genre LIKE "%Drama%"
GROUP BY name
)
